...
export class BillingComponent implements OnInit {
  ...
  protected async generar(f: CuentaDelMes): Promise<void> {
    ...
    this.api.generarPrecuentas(f.periodo, f.profesional_id).subscribe({
      next: (r) => {
        this.ocupada.set(null);
        const omitida = r.data.omitidas?.[0];
        if (!r.data.generadas?.length && omitida) {
          this.alerts.warning('No se generó la cuenta', omitida.motivo);
        } else {
          this.alerts.success('Cuenta de cobro generada', r.message);
        }
        this.cargar();
      },
      error: (err) => {
  ...
  /** PRE-07 · Tras revisar un rechazo, el admin la deja aceptada. */
  protected async marcarAceptada(f: CuentaDelMes): Promise<void> {
    if (!f.precuenta_id) return;
    const ok = await this.alerts.confirm({
      title: 'Marcar como aceptada',
      message:
        `La cuenta de ${f.profesional_nombre} (${this.periodoLargo(f.periodo)}) quedará como aceptada ` +
        `y pasará a la pestaña "Aceptadas".`,
      confirmText: 'Marcar aceptada',
    });
    if (!ok) return;
    this.api.setEstadoPrecuenta(f.precuenta_id, 'aceptada').subscribe({
      next: () => {
        this.alerts.success('Cuenta aceptada', 'Queda lista para el proceso de pago.');
        this.cargar();
      },
