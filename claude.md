...
**Ya no queda código pendiente de la tanda**: lo que falta son las nueve
decisiones con el cliente (§8 del plan) y verla funcionando en la app.
...
> **eje de facturación**. Ninguna se ha visto funcionar dentro de la app todavía.
...
| M9 Cuentas de cobro (PRE-01..09) | ✅ backend + frontend · ⚠️ **desde ago-2026 la cuenta puede llevar VIÁTICOS**: `precuenta_items.viaticos` + `precuentas.total_viaticos`, como línea aparte de los honorarios. `total_monto` los incluye (es lo que se paga), `monto` sigue siendo horas × valor hora, y **`ordenes_servicio.valor_cobro_total` NO** — es una columna generada y meter ahí un reembolso dejaría un valor hora falso. El correo, el PDF y el enlace público desglosan · **una fila por profesional y mes**, que aparece cuando se **aceptan los soportes** de una orden (no con un cierre mensual manual); se entra por año y mes, las aceptadas van en su pestaña y **no se puede generar una cuenta en cero**. **Aceptar o rechazar solo lo hace el profesional** desde su enlace (el servidor rechaza el cambio de estado); ante un rechazo el administrador **rehace** la cuenta y la reenvía. CFG-05 (día de corte) vive ahora en Configuración → Sistema |
...
| M13 Estado de facturación (petición 6, ago-2026) | ✅ backend + frontend · **eje independiente del ciclo de la OS**: `sst.estado_cobro` (NO FACTURADA → RADICADA → APROBADA → FACTURADA → PAGADA) con historial propio (`sst.historial_cobro_orden`). Se marca **en lote** desde `/ordenes` (`PATCH /orders/cobro`, admin y **contador**) y **solo sobre órdenes FINALIZADAS**; `FACTURADA` exige número de factura. **Nada lo mueve solo**: radicar ante la ARL es un acto de la contadora, no un efecto de la plataforma |
