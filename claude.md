...
**Al 23-ago-2026 están construidas y migradas LAS CINCO fases** (los enumerados
del AT-031 de Bolívar; la matriz de formatos y soportes por ARL; los viáticos;
el profesional registrado ante la ARL con su suplente; el eje de facturación),
**más los cuatro ajustes que el cliente pidió sobre lo entregado** (§10 del
plan): el estado de cobro se cambia solo desde el icono de la fila, el eje se
queda en **dos** estados, su diálogo es estrecho y los **viáticos se eligen de un
catálogo** en vez de escribirse.
**Ya no queda código pendiente de la tanda**: lo que falta son las decisiones
con el cliente (§8 del plan) y verla funcionando en la app.
...
Nada de las cinco fases se ha visto funcionar **dentro de la aplicación**: todo
lo verificado lo está por script —contra la BD real y con `ROLLBACK`—, llamando
a los endpoints con un JWT firmado a mano contra una instancia temporal en
`:4010`, o mirando los PDF generados.
...
> 🆕 **22/23-ago-2026 · tanda cerrada: seis peticiones del cliente en cinco fases.**
> El tablero es **`docs/plan-peticiones-22-ago-2026.md`** — ahí van el estado de
> cada una, lo que se construyó y las **nueve decisiones pendientes con el
> cliente**. **Las cinco fases están construidas y migradas**: los dos enumerados
> del AT-031 de Bolívar, la matriz de formatos y soportes por ARL, los
> **viáticos**, el **profesional registrado ante la ARL con su suplente** y el
> **eje de facturación**. El **§10** recoge los cuatro ajustes del 23-ago sobre lo
> entregado. Ninguna se ha visto funcionar dentro de la app todavía.
...
| M2 Importación y extracción IA (IMP-01..09) | ✅ backend + frontend · se cargan **varios archivos a la vez** (un lote por archivo) y cada orden se puede guardar suelta · confirmar **materializa la OS** directamente en SIN PROGRAMAR (ya no hay paso de "validar" en la bandeja) · el **Excel SIPAB de Bolívar se lee por nombre EXACTO de columna** (`SIPAB_HEADERS` en `extraction.service.js`): la ubicación se parte en ciudad/dirección/contacto y las fechas `01/aug/2026` se normalizan a ISO; comprobar un archivo real con `node --import tsx scripts/verificar-sipab.mjs "<.xlsx>"` · **"Act Programadas" solo son horas si `Unidad Medida` dice HORAS**; si no, el campo llega vacío y las horas las escribe quien revisa (obligatorio para guardar), porque "Hora Programada" es la hora de INICIO de la visita, no una duración · ⚠️ **el SIPAB no trae fecha de vencimiento** y la app la exige: **se escribe a mano, orden por orden** (decidido con el cliente el 19-ago-2026; ver HANDOFF §3) |
| M3 Estados y auditoría (EST-01..06) | ✅ backend + frontend · el **código OS-YYYY-NNNN** se reparte bajo `pg_advisory_xact_lock` y desde el máximo usado (`siguienteCodigoOS`), no con `count(*)+1`: con dos archivos confirmándose en paralelo se repetía el número y media tanda no entraba · ⚠️ **el ciclo son CUATRO estados** (ago-2026, a pedido del cliente): **SIN PROGRAMAR → PROGRAMADA → EJECUTADA → FINALIZADA**. EJECUTADA la pone el **profesional** al subir los soportes; **FINALIZADA** la pone el **administrador** al aceptarlos, y es el cierre real (de él cuelgan la encuesta y la cuenta de cobro; no tiene salida). Se eliminaron EN VERIFICACIÓN y CANCELADA (divergen de EST-01). Una OS solo pasa a PROGRAMADA cuando las franjas cubren **todas** sus horas; `EJECUTADA → PROGRAMADA` es el rechazo de soportes, la única marcha atrás (diverge de EST-06). "Deshabilitada" no es un estado de la OS: es el soft-delete del borrador |
| M4 Formatos (FOR-01..06) | ✅ backend + frontend · el correo de asignación adjunta los **formatos oficiales de la ARL ya diligenciados**. ⚠️ **Desde ago-2026 QUÉ formatos salen depende de una matriz** (`services/entrega-arl.service.js`): ARL × tipo de actividad × horas × modalidad. Una asesoría de Bolívar solo lleva AT-031; una capacitación **virtual** no lleva AT-028 (lo prohíbe el comunicado de la ARL); una asesoría de AXA ≤16 h lleva ficha de gestión y >16 h informe técnico. Cada formato tiene **`alcance`**: los de sesión salen uno por franja, los de orden (informes, fichas) una sola vez. Los `.docx`/`.xls`/`.pptx` se adjuntan **sin tocar** (`modo: 'adjunto'`). El registro de archivos y cómo se rellena cada uno está en `formatos-arl.service.js` + `assets/formatos-arl/README.md` |
| M5 Asignación y reprogramación (ASG-01..08) | ✅ backend + frontend · ASG-02 se programa sobre una agenda semanal y la visita se reparte en **franjas** (`sst.franjas_visita`; `fecha_programada` = inicio de la primera) · ASG-05 = invitación .ics adjunta al correo (no API de Google Calendar) · ASG-08 = el panel del profesional muestra su agenda · **ASG-06 (WhatsApp) fuera: el FRS lo deja en Fase 3 y omisible** · ⚠️ **desde ago-2026 los formatos pueden salir a nombre de OTRO profesional**: Bolívar solo acepta radicados a nombre de quien tiene registrado (`sst.profesionales_arl`), así que `ordenes_servicio.profesional_formatos_id` dice quién FIRMA y **`profesional_asignado_id` sigue siendo QUIEN EJECUTA** — el correo, el `.ics`, el enlace de soportes, la agenda, la cuenta de cobro y la encuesta son todos del ejecutor. El único sitio donde los dos papeles se separan es `generateOrderDocuments` |
| M6 Soportes por enlace público (SUP-01..07) | ✅ backend + frontend · SUP-07 lista los archivos ya enviados en la agenda del profesional · **SUP-05 diverge: subir soportes deja la OS EJECUTADA**, no EN VERIFICACIÓN · ⚠️ **las casillas ya NO son las mismas para todas las órdenes** (ago-2026): salen de la misma matriz que los formatos y se congelan en `soportes_requeridos` al asignar — una asesoría de Bolívar no pide registro fotográfico, una asistencia técnica pide `informe`. `casillasDeOrden()` en `soportes.service.js`; `CATEGORIAS_SOPORTE` es solo el catálogo · cada archivo viaja en el campo de **su casilla** (`acta`/`asistencia`/`evidencias`/`informe`), se **comprime** antes de guardarse (`services/compress.service.js`, 95-96 % en fotos; los PDF ya optimizados no bajan) y se almacena con nombre propio (`acta.pdf`), nunca con el del móvil |
...
| M9 Cuentas de cobro (PRE-01..09) | ✅ backend + frontend · ⚠️ **desde ago-2026 la cuenta puede llevar VIÁTICOS** (que se ELIGEN de `sst.tipos_viatico`, no se escriben): `precuenta_items.viaticos` + `precuentas.total_viaticos`, como línea aparte de los honorarios. `total_monto` los incluye (es lo que se paga), `monto` sigue siendo horas × valor hora, y **`ordenes_servicio.valor_cobro_total` NO** — es una columna generada y meter ahí un reembolso dejaría un valor hora falso. El correo, el PDF y el enlace público desglosan · **una fila por profesional y mes**, que aparece cuando se **aceptan los soportes** de una orden (no con un cierre mensual manual); se entra por año y mes, las aceptadas van en su pestaña y **no se puede generar una cuenta en cero**. **Aceptar o rechazar solo lo hace el profesional** desde su enlace (el servidor rechaza el cambio de estado); ante un rechazo el administrador **rehace** la cuenta y la reenvía. CFG-05 (día de corte) vive ahora en Configuración → Sistema |
| M10 Reportes (RPT-01..05, 07) | ✅ backend + frontend · dashboard, buscador NL, vencidas, satisfacción, horas, **cobro** y exportación a Excel · ⚠️ **RPT-06 (Cartera) se retiró** el 19-ago-2026 a pedido del cliente. La pestaña **Cobro** de ago-2026 NO es su vuelta: aquello era un reporte con tres fechas que nadie llenaba, esto es el **eje de facturación de la orden** (`GET /reports/cartera` sigue sin existir). ⚠️ Su cifra es `valor_total` —lo que se le cobra a la ARL—, **no** `valor_cobro_total`, que es lo que se le paga al profesional |
| M13 Estado de facturación (petición 6, ago-2026) | ✅ backend + frontend · **eje independiente del ciclo de la OS**: `sst.estado_cobro` con historial propio (`sst.historial_cobro_orden`). ⚠️ **Son DOS estados: NO FACTURADA y FACTURADA** — nació con cinco y el cliente retiró RADICADA, APROBADA y PAGADA el 23-ago-2026 (la lista está copiada en el enum de `schema.sql`, en `orders.routes.js` y en `core/models.ts`: se tocan los tres o ninguno). Se cambia **de una orden en una y solo desde el icono de facturación de la fila** en `/ordenes` (`PATCH /orders/cobro`, admin y **contador**), **solo sobre FINALIZADAS**; `FACTURADA` exige número de factura. Ni el detalle ni la edición lo ofrecen. **Nada lo mueve solo**: radicar ante la ARL es un acto de la contadora, no un efecto de la plataforma |
| M11 Notificaciones (NOT-01..04) | ✅ correos + campanita interna |
| M12 Configuración (CFG-01..05) | ✅ completo · CFG-02 = maestro `sst.empresas` + `/empresas` · CFG-03 = plantillas editables **ocultas** (las tres ARL traen sus formatos oficiales; el generador sigue en el backend) · CFG-04 = catálogo de tipos de orden con su valor hora · CFG-05 = día de corte que **avisa por la campanita** del mes anterior sin cobrar, no automatiza · **catálogo de tipos de viático** (`sst.tipos_viatico`, nombre + valor) del que sale el importe al cargar una orden: **nace vacío**, y sin filas la única opción es "No aplica" |
...
| `/dashboard` | `pages/dashboard` | KPIs y distribución por ARL (RPT-01/02). "Órdenes recientes" muestra **10 filas** con las mismas columnas que Órdenes y un botón "Ver todo"; **pulsar una fila abre esa orden en `/ordenes?os=<id>`** (ya no hay panel lateral de asignación). |
| `/importar` | `pages/import` | Carga de **varios** Excel/PDF a la vez (un lote por archivo), extracción IA, revisión y confirmación (M2). El **tipo de orden es obligatorio**: se elige en cada fila de la vista previa (el pipeline lo preselecciona cuando el título lo dice) y sin él no se guarda. Cada fila se puede **guardar suelta** (`POST /drafts/:id/confirm`) además del "Guardar todo". Las **duplicadas no se listan**: se descartan y se avisan con el estado de la OS que ya existe. Al ELEGIR los archivos se comprueba **sin IA** (`POST /imports/precheck`) si la orden ya está cargada; la que sí, se aparta del selector —con "Procesar de todos modos" por si es un falso positivo— y nunca llega al modelo. Cada selección **se suma** a la tanda (el selector del navegador solo deja marcar dentro de una carpeta) y cada archivo se puede quitar suelto. |
| `/ordenes` (legado: `/validacion`) | `pages/validation` | **Vista central.** Bandeja con pestañas **por estado** (Todas · Sin programar · Programadas · Ejecutadas · Finalizadas · Deshabilitadas; *Ejecutadas* es la bandeja de revisión y *Finalizadas* el archivo), detalle y edición **en cualquier estado** (sobre el borrador si aún no se validó; sobre la OS —`PUT /orders/:id`— en cuanto existe), cambio de estado + historial (M3), asignación y reprogramación sobre una **agenda semanal** con la visita repartida en franjas (M5) —no deja marcar sobre horarios ocupados ni pasarse de las horas de la orden—, visor de soportes y aceptar/**rechazar por documento** —se marca cuál se devuelve y el portal solo abre esas casillas— (M7). El modal de asignación deja además elegir **a nombre de qué profesional registrado salen los formatos** cuando el ejecutor no lo está (ASG · suplente). Columna y **filtro de estado de cobro**, un **icono de facturación por fila** (solo en las FINALIZADAS) que es el **único** sitio desde el que se cambia, y bloque "Facturación a la ARL" **de solo consulta** con su historial en el detalle. El detalle lleva además el desplegable de **viáticos**, que se eligen del catálogo y no se escriben. Acepta `?os=<id>`. |
| `/informes` | `pages/reports` | Centro de reportes (M10), seis pestañas: Órdenes, Profesionales, Satisfacción (ENC-05 + RPT-04), Vencidas (RPT-03), Horas (RPT-05) y **Cobro** (estado de facturación de lo finalizado). Todas exportan a Excel y PDF (RPT-07). |
...
| `/profesionales` | `pages/professionals` | CRUD de asesores y su agenda de ocupaciones (CFG-01), con su **desempeño**: órdenes ejecutadas y promedio en estrellas de las encuestas (ENC-05). Al pulsar las estrellas se abre el detalle: cada encuesta con su nota y la observación del cliente. La encuesta es opcional, así que el promedio siempre va con el número de respuestas. Columna **"Registro ARL"** con las ARL ante las que está registrado (naranja = vigencia vencida) y modal propio para marcarlo, con su código y su vigencia: **de ahí sale quién puede firmar los formatos de una orden**. |
| `/configuracion` | `pages/settings` | Perfil (guarda con `PUT /auth/me`) · **Preferencias del Sistema** (umbral de confianza, día de corte, el catálogo **CFG-04 de tipos de orden con su valor hora** —que alimenta la cuenta de cobro— y el catálogo de **tipos de viático con su valor**, del que sale el importe al cargar una orden) · **Formatos y encuesta** (ENC-03: enunciados de la encuesta; las plantillas de PDF están ocultas tras `plantillasVisibles`) · usuarios internos y matriz de Roles y permisos. |
...
| `/precuentas` | `pages/billing` | **Cuentas de cobro.** Año + tira de meses arriba; debajo, una fila por profesional y mes (columna Mes) con lo que hay por pagar. Las filas nacen al **aceptar los soportes** de una orden (Órdenes → Verificar soportes → Aceptar; ahí la OS pasa a FINALIZADA); "generar" congela la cifra y emite el documento, y "enviar" se lo manda al profesional con el enlace para aceptar o rechazar. Una orden ya incluida en una cuenta deja de contar como pendiente, y el trabajo finalizado después de cerrarse la cuenta del mes se cobra en una **cuenta complementaria** (puede haber varias por profesional y mes). Sobre una cuenta **rechazada**, "generar" la **rehace** (mismo registro, tarifas de hoy) y vuelve a quedar lista para enviar. Acepta `?cuenta=<id>` y `?periodo=AAAA-MM` desde la campanita: recarga, se coloca en la pestaña de esa cuenta y resalta la fila. Pestañas: Por cobrar · Aceptadas · Tarifas por actividad (PRE-02). Visible para admin, contador y auditor. |
...
- **Todo lo que se abre encima de una vista es un modal centrado**
  ...
  lateral: fichas, formularios de alta y edición, detalles. **Ya no queda ningún
  `.drawer`**: el del dashboard se retiró el 20-ago-2026 y pulsar una fila de
  "Órdenes recientes" lleva a esa orden en `/ordenes?os=<id>`.
- **Los campos extraídos se corrigen con las reglas de `shared/campos-orden.ts`**,
  que comparten los dos modales de revisión (Importar y Órdenes): qué se puede
  teclear en cada campo (solo letras, solo dígitos, correo, decimal), su
  validación, y la **confianza mostrada** — un campo corregido a mano vale 100 %
  en el acto y pierde el aviso de baja confianza; vaciarlo lo devuelve.
